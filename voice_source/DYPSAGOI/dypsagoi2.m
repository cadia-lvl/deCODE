% Using iaif2.m and LTFAT.

function [gci,goi,gcic,goic,gdwav,udash,crnmp] = dypsagoi2(s,fs,opt)
...
[decomposition, oa2, oa3] = wfilt_spline(1, 5);
[swa swd] = swtalign([udash; zeros(nU-nu,1)],nlev,decomposition);
...
for k = 1:n
    ...
    % Decomposition.
    swd(k,:) = wkeep1(conv(x,hi),s,lf);   % Default last arg was lf+1.
    swa(k,:) = wkeep1(conv(x,lo),s,lf);
...
function part = wkeep1(vector, length_extracted, first)

part = vector(first:first+length_extracted-1);


function upsampled = dyadup(vector)

vector = vector(:);
length_vector = length(vector);
upsampled = zeros(length_vector*2, 1);
upsampled(1:2:end) = vector;
